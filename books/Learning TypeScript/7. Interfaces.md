...
- type aliases와 interfaces의 주요한 차이점
  - interface들은 서로 병합되어 커질 수 있음 -> built-in globals 또는 npm packages와 같은 third-party 코드와 작업할 때 특히 유용
  - interface는 class 선언 구조를 type check 할 수 있음
  - 보통 interface가 type checking이 더 빠름 -> type aliases는 새로운 object literal의 동적인 copy-and-paste를 하는 반면, interface는 쉽게 캐싱가능한 named type을 선언함
  - interface가 error message를 이해하기 더 쉬움 -> type aliases는 unnamed object literal로 취급되지만 interface는 named object로 취급되기 때문에
...
- 두 syntax의 주요한 차이점
  - method는 `readonly`로 선언될 수 없음
  - interface merging은 이 두 syntax를 다르게 취급함(이 챕터 뒤에서 자세히)
  - type에서 수행되는 일부 작업이 이 두 syntax를 다르게 다르게 취급함(ch15에서 자세히)
- function이 `this`를 참조하는 경우(대부분 class)는 method 사용. 나머지는 property 사용
...
- 어떤 interface에서 call signature가 선언한 방법으로 호출될 수 있는 값만이 그 interface에 할당 가능함
